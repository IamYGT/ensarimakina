<?php require("../include/baglan.php");include("../include/fonksiyon.php");
		...
		define('TABLE',"nedenildi");
		define('AREA',"nedenildi");
		if(!isset($do)) $do = null;
		$sayfa = (isset($q) ? $q : 1);
							...
							<div id="example5_wrapper" class="dataTables_wrapper no-footer">
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									<thead>
										...
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 115.6px;">İsim Soyisim</th>
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Company: activate to sort column ascending" style="width: 215.6px;">Resim 65x65</th>
...
?>
										<tr id="item-<?php echo $row["haber_ust_id"]; ?>" role="row" class="even">
											...
											<td class=""><?php if(isset($row["haber_resim"])){ ?><img class="mr-3 " width="60" src="../uploads/nedenildi/<?php echo $row["haber_resim"]; ?>" alt="<?php echo $row["haber_baslik"]; ?>"><?php }else{ ?><img class="mr-3 " width="60" src="images/no_images.png" alt="<?php echo $row["haber_baslik"]; ?>"><?php } ?></td>
 											<td class="sorting_1">
												<div class="d-flex">
				...
				if(isset($submitControl)){
					if(!empty($_POST['haber_baslik']) || !empty($_POST['haber_aciklama'])) {
						...
						if ($upload->uploaded) {
								...
								$upload->process("../uploads/nedenildi");
							if ($upload->processed){
					...
					<div class="col-xl-8 col-xxl-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Ne Denildi?</h4>
                            </div>
                            <div class="card-body">
													...
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> İsim Soyisim</label>
																<input type="text" name="haber_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="İsim Soyisim" required="">
															</div>
														</div>

														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Yorum</label>
																<textarea class="form-control" name="haber_aciklama[<?php echo $LANGUAGE_CODE; ?>]"></textarea>
															</div>
														</div>
													</div>
...
?>
				...
				<form action="#" method="POST" enctype="multipart/form-data" id="form" style="display: inline-flex; ">
					<div class="col-xl-8 col-xxl-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Ne Denildi? Düzenle</h4>
                            </div>
                            <div class="card-body">
								<div class="step-app" id="demo">
									<ul class="step-steps">
												...
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> İsim Soyisim</label>
																<input type="text" name="haber_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="İsim Soyisim" required="" value="<?php echo GetTableValue("haber_baslik",TABLE,"where haber_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														...
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Yorum</label>
																<textarea class="form-control" name="haber_aciklama[<?php echo $LANGUAGE_CODE; ?>]"><?php echo GetTableValue("haber_aciklama",TABLE,"where haber_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?></textarea>
															</div>
														</div>
													</div>
													<input type="hidden" name="form_<?php echo $LANGUAGE_CODE; ?>" value="<?php echo $LANGUAGE_CODE; ?>" />
                            ...
                            <div class="card-body p-3">
								...
								<?php }else{?>
										<div class="fileinput fileinput-exists" data-provides="fileinput">
											...
											<div class="fileinput-preview fileinput-exists thumbnail" style="max-width: 303px; max-height: 190px; line-height: 190px;"><img src="../uploads/nedenildi/<?php echo $row_info["haber_resim"]; ?>" class="img-fluid img-thumbnail"></div>
											<div>
												<span class="btn btn-primary btn-sm btn-file">
													<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
													<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
