<?php require("../include/baglan.php");include("../include/fonksiyon.php"); 
		...
		define('TABLE',"ekibimiz");
		define('AREA',"ekibimiz");
		if(!isset($do)) $do = null; 
		$sayfa = (isset($q) ? $q : 1);
						...
						<div class="table-responsive"> 
							<div id="example5_wrapper" class="dataTables_wrapper no-footer">
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									<thead>
										<tr role="row"> 
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 1%;">Row</th>
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 115.6px;">Title</th>
										...
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Postition: activate to sort column ascending" style="width: 82px;">Date</th>
...
<?php
									$list = $db->query("SELECT * FROM ".TABLE." WHERE dil_id = '".LANGUAGE_DEFAULT."' ORDER BY row ASC LIMIT $baslangic,$limit"); 
										if ($list->rowCount()){
											foreach($list as $row){ 
?>
										<tr id="item-<?php echo $row["ekip_ust_id"]; ?>" role="row" class="even"> 
											<td class="sortable ui-sortable-handle" style="width:20px"><i class="fa fa-sort"></i></td>
											<td class=""><?php echo $row["ekip_baslik"]; ?></td>
											<td class=""><?php if(isset($row["ekip_resim"])){ ?><img class="mr-3 " width="60" src="../uploads/news/<?php echo $row["ekip_resim"]; ?>" alt="<?php echo $row["ekip_baslik"]; ?>"><?php }else{ ?><img class="mr-3 " width="60" src="images/no_images.png" alt="<?php echo $row["ekip_baslik"]; ?>"><?php } ?></td>
											<td class=""><?php echo $row["ekip_tarih"]; ?></td>
											<td class="sorting_1">
												<div class="d-flex">
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=edit&id=<?php echo $row["ekip_ust_id"]; ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>
													<a class="contact-icon" href="<?php echo AREA; ?>?do=delete&id=<?php echo $row["ekip_ust_id"]; ?>"><i class="fas fa-trash-alt"></i></a>
												</div>
											</td>
										</tr> 
<?php 	
...
<?php 
				...
				if(isset($submitControl)){
					if(!empty($_POST['ekip_baslik']) || !empty($_POST['ekip_aciklama'])) {
						$LastID = $db->query("SELECT ekip_id FROM ".TABLE." ORDER BY ekip_id DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
						$LastID = $LastID["ekip_id"]+1;
						$upload = new Upload($_FILES['ekip_resim']);
						if ($upload->uploaded) { 
								$upload->file_auto_rename = true;
								$upload->process("../uploads/teams");
							if ($upload->processed){  
								$ekip_resim = $upload->file_dst_name;
							} else { 
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET ekip_baslik = ?, ekip_seo = ?, ekip_aciklama = ?, ekip_resim = ?, dil_id = ?, ekip_durum = ?, ekip_ust_id = ?");
								$insert->execute(array($ekip_baslik[$key], Seo_Link_Cevir($ekip_baslik[$key]), $ekip_aciklama[$key], $ekip_resim, $key, $ekip_durum, $LastID));
													...
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																...
																<input type="text" name="ekip_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Title" required="">
															</div>
														</div>   
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Açıklama</label>
																<textarea class="form-control ckeditor" name="ekip_aciklama[<?php echo $LANGUAGE_CODE; ?>]"></textarea>
															</div>
														</div>
													</div>
                        ...
                        <div class="card">
                            <div class="card-body p-3"> 
									...
									<div class="fileinput fileinput-new" data-provides="fileinput">
										...
										<div>
											<span class="btn btn-primary btn-sm btn-file">
												...
												<input type="file" name="ekip_resim" accept="image/*" id="ekip_resim">
											</span>
								...
								<div class="form-group"> 
									...
									<select name="ekip_durum" class="form-control">
...
<?php 
				...
				$row_check = $db->prepare("SELECT * FROM ".TABLE." WHERE ekip_ust_id = ?");
				$row_check->execute(array($id)); 
				if ($row_check->rowCount() > 0) {
					if(isset($submitControl)){
						if(!empty($_POST['ekip_baslik']) || !empty($_POST['ekip_aciklama'])) {
							$image_info = $db->query("SELECT * FROM ".TABLE." WHERE ekip_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
								$upload = new Upload($_FILES['ekip_resim']);  
								if(!isset($_FILES['ekip_resim']['name'])) {
									$ekip_resim = null;
								} else if(!empty($_FILES['ekip_resim']['name'])) {
									$upload->file_auto_rename = true; 
									$upload->process("../uploads/teams");
									if ($upload->processed) {
										$ekip_resim = $upload->file_dst_name;
									...
									$ekip_resim = $image_info["ekip_resim"];
								} 
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									$key = str_replace("form_","",$k);
									$update = $db->prepare("UPDATE ".TABLE." SET  ekip_baslik = ?, ekip_seo = ?, ekip_aciklama = ?, ekip_resim = ?, ekip_durum = ? WHERE ekip_ust_id = ? AND dil_id = ?");
									$update->execute(array($ekip_baslik[$key], Seo_Link_Cevir($ekip_baslik[$key]), $ekip_aciklama[$key], $ekip_resim, $ekip_durum, $id, $key));
								}
							}
							if ($update) {
					...
					$row_info = $db->query("SELECT * FROM ".TABLE." WHERE ekip_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
?>
				...
				<form action="#" method="POST" enctype="multipart/form-data" id="form" style="display: inline-flex; "> 
					<div class="col-xl-8 col-xxl-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Ekibi Düzenle</h4>
                            </div>
                            <div class="card-body"> 
								<div class="step-app" id="demo">
									<ul class="step-steps">
												...
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Title</label>
																<input type="text" name="ekip_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Title" required="" value="<?php echo GetTableValue("ekip_baslik",TABLE,"where ekip_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Açıklama</label>
																<textarea class="form-control ckeditor" name="ekip_aciklama[<?php echo $LANGUAGE_CODE; ?>]"><?php echo GetTableValue("ekip_aciklama",TABLE,"where ekip_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?></textarea>
															</div>
														</div>
													</div>
													<input type="hidden" name="form_<?php echo $LANGUAGE_CODE; ?>" value="<?php echo $LANGUAGE_CODE; ?>" />
					...
					<div class="col-xl-4 col-xxl-4">
                        <div class="card">
                            <div class="card-body p-3">  
								...
								<label>Resim</label>
								<small class="form-text text-muted">Seçmiş olduğunuz resim veri içeriğinde kullanılmaktadır.</small> 
								<?php if(empty($row_info["ekip_resim"])) {?>
										<div class="fileinput fileinput-new" data-provides="fileinput">
											<div class="fileinput-preview thumbnail" data-trigger="fileinput" style="width: 100%;height: 190px;line-height: 190px;">
								...
								<?php }else{?> 
										<div class="fileinput fileinput-exists" data-provides="fileinput">
											...
											<div class="fileinput-preview fileinput-exists thumbnail" style="max-width: 303px; max-height: 190px; line-height: 190px;"><img src="../uploads/teams/<?php echo $row_info["ekip_resim"]; ?>" class="img-fluid img-thumbnail"></div>
											<div>
												<span class="btn btn-primary btn-sm btn-file">
													<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
													<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
													<input type="file" name="ekip_resim" id="ekip_resim" >
								...
								<div class="form-group"> 
									<label for="">Yayın Durumu</label>
									<select name="ekip_durum" class="form-control">
										<option value="1" <?php if ($row_info["ekip_durum"] == "1") { echo "selected";}?>>Aktif</option>
										<option value="2" <?php if ($row_info["ekip_durum"] == "2") { echo "selected";}?>>Pasif</option> 
									</select>
								</div> 
                            </div>
                        </div>
...
<?php 
				...
				$row_check->execute(array($id)); 
				if ($row_check->rowCount() > 0) {
					$update = $db->exec("DELETE FROM ".TABLE." WHERE ekip_ust_id = {$id};");
					if($update) {
		...
		$( "#sortable" ).sortable({
			...
			stop: function (event, ui) {
				...
				$.ajax({
					...
					dataType: "json",
					data: data,
					url: "/include/ajax.php?do=teams&action=row",
