...
<?php
				...
				if(isset($submitControl)){
					if(!empty($_POST['slayt_baslik']) || !empty($_POST['slayt_aciklama'])) {
						...
						$upload = new Upload($_FILES['slayt_resim2']);
						if ($upload->uploaded) {
								$upload->file_auto_rename = true;
								$upload->process("../uploads/sliders");
							if ($upload->processed){
								$slayt_resim2 = $upload->file_dst_name;
							} else {
								$error = Bilgilendirme::Hata("Hay Akis! Bir hata meydana geldi.".$upload->error);
							}
						}
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET slayt_baslik = ?, slayt_aciklama = ?,  slayt_oguc = ?,  slayt_sguc = ?,  slayt_otork = ?,  slayt_stork = ?, slayt_buton = ?, slayt_butonlink = ?, slayt_link = ?, slayt_resim = ?,  slayt_resim2 = ?, dil_id = ?, slayt_durum = ?, slayt_ust_id = ?");
								$insert->execute(array($slayt_baslik[$key], $slayt_aciklama[$key], $slayt_buton[$key],  $slayt_oguc[$key],  $slayt_sguc[$key],  $slayt_otork[$key],  $slayt_stork[$key], $slayt_butonlink[$key],  $slayt_link[$key],  $slayt_resim, $slayt_resim2, $key, $slayt_durum, $LastID));
													...
													<div class="row">
														...
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																...
																<input type="text" name="slayt_aciklama[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Açıklama" required="">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
														...
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																...
																<input type="text" name="slayt_butonlink[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Buton Adresi" required="">
															</div>
														</div>
					...
					if(isset($submitControl)){
						if(!empty($_POST['slayt_baslik']) || !empty($_POST['slayt_aciklama'])) {
							$image_info = $db->query("SELECT * FROM ".TABLE." WHERE slayt_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
								...
								$upload = new Upload($_FILES['slayt_resim2']);
								if(!isset($_FILES['slayt_resim2']['name'])) {
									$slayt_resim2 = null;
								} else if(!empty($_FILES['slayt_resim2']['name'])) {
									$upload->file_auto_rename = true;
									$upload->process("../uploads/sliders");
									if ($upload->processed) {
										$slayt_resim2 = $upload->file_dst_name;
									...
									$slayt_resim2 = $image_info["slayt_resim2"];
								}
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									$key = str_replace("form_","",$k);
									$update = $db->prepare("UPDATE ".TABLE." SET  slayt_baslik = ?, slayt_aciklama = ?,   slayt_oguc = ?,  slayt_sguc = ?,  slayt_otork = ?,  slayt_stork = ?, slayt_buton = ?, slayt_butonlink = ?, slayt_link = ?, slayt_resim = ?, slayt_resim2 = ?, slayt_durum = ? WHERE slayt_ust_id = ? AND dil_id = ?");
									$update->execute(array($slayt_baslik[$key], $slayt_aciklama[$key],   $slayt_oguc[$key],  $slayt_sguc[$key],  $slayt_otork[$key],  $slayt_stork[$key], $slayt_buton[$key], $slayt_butonlink[$key], $slayt_link[$key], $slayt_resim,  $slayt_resim2, $slayt_durum, $id, $key));
								}
							}
							if ($update) {
												...
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														...
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Açıklama</label>
																<input type="text" name="slayt_aciklama[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Açıklama" required="" value="<?php echo GetTableValue("slayt_aciklama",TABLE,"where slayt_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>

														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Button İsmi</label>
																<input type="text" name="slayt_buton[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Button İsmi" required="" value="<?php echo GetTableValue("slayt_buton",TABLE,"where slayt_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
