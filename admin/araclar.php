						...
						<div class="table-responsive"> 
							<div id="example5_wrapper" class="dataTables_wrapper no-footer">
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									<thead>
										<tr role="row"> 
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 25%">Title</th>  
				...
				if(isset($submitControl)){
					if(!empty($_POST['marka'])) {
						...
						if ($upload->uploaded) { 
							...
							$upload = new Upload($_FILES['resim2']);
						if ($upload->uploaded) { 
								$upload->file_auto_rename = true;
								$upload->process("../uploads/cars");
							if ($upload->processed){  
								$resim2 = $upload->file_dst_name;
							} else { 
								$error = Bilgilendirme::Hata("Hay Aksi! Bir hata meydana geldi.".$upload->error);
							}
						}
						$insert = $db->prepare("INSERT INTO ".TABLE." SET marka = ?, resim = ?, resim2 = ?, link = ?, gizli_ozellik = ?, marka_durum = ?");
						$insert->execute(array($marka, $resim, $resim2, Seo_Link_Cevir($marka), $gizli_ozellik, $marka_durum));
								...
								<div class="row">
									<div class="col-lg-12 mb-2">
										<div class="form-group">
											...
											<input type="text" name="marka" class="form-control" placeholder="Title" required="">
										</div> 
									</div>
								</div>
									...
									<div class="fileinput fileinput-new" data-provides="fileinput">
										...
										<div>
											<span class="btn btn-primary btn-sm btn-file">
												<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
												<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
												<input type="file" name="resim2" accept="image/*" id="resim2">
											</span>
											<a href="#" class="btn btn-primary btn-sm fileinput-exists" data-dismiss="fileinput"><span class="fui-trash"></span>Vazgeç</a>
										</div>
									</div>
					...
					if(isset($submitControl)){
						if(!empty($_POST['marka'])) { 
							...
							if(!isset($_FILES['resim']['name'])) {
								...
								$upload = new Upload($_FILES['resim2']);  
							if(!isset($_FILES['resim2']['name'])) {
								$resim2 = null;
							} else if(!empty($_FILES['resim2']['name'])) {
								$upload->file_auto_rename = true; 
								$upload->process("../uploads/cars");
								if ($upload->processed) {
									$resim2 = $upload->file_dst_name;
								...
								$resim2 = $image_info["resim2"];
							}
							
							$update = $db->prepare("UPDATE ".TABLE." SET marka = ?, resim = ?, resim2 = ?, link = ?, gizli_ozellik = ?, marka_durum = ?, marka_id = ?");
							$update->execute(array($marka, $resim, $resim2, Seo_Link_Cevir($marka), $gizli_ozellik, $marka_durum, $id));
							if ($update) {
                            ...
                            <div class="card-body">   
								<div class="row">
									<div class="col-lg-12 mb-2">
										<div class="form-group">
											<label class="text-label">Title</label>
											<input type="text" name="marka" class="form-control" placeholder="Title" required="" value="<?php echo $row_info["marka"]; ?>">
										</div> 
									</div>
								</div>
								<div class="step-footer pull-right">
                            ...
                            <div class="card-body p-3">  	
								...
								<?php }else{?> 
										<div class="fileinput fileinput-exists" data-provides="fileinput">
											...
											<div class="fileinput-preview fileinput-exists thumbnail" style="max-width: 303px; max-height: 190px; line-height: 190px;"><img src="../uploads/cars/<?php echo $row_info["resim2"]; ?>" class="img-fluid img-thumbnail"></div>
											<div>
												<span class="btn btn-primary btn-sm btn-file">
													<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
													<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
