...
<?php
									$list = $db->query("SELECT * FROM ".TABLE." WHERE dil_id = '".LANGUAGE_DEFAULT."' ORDER BY row ASC LIMIT $baslangic,$limit");
										if ($list->rowCount()){
											foreach($list as $row){
?>
										<tr id="item-<?php echo $row["proje_ust_id"]; ?>" role="row" class="even">
											<td class="sortable ui-sortable-handle" style="width:20px"><i class="fa fa-sort"></i></td>
											<td class=""><?php echo $row["proje_baslik"]; ?></td>
											<td class=""><?php if(isset($row["proje_resim"])){ ?><img class="mr-3 " width="60" src="../uploads/projects/<?php echo $row["proje_resim"]; ?>" alt="<?php echo $row["proje_baslik"]; ?>"><?php }else{ ?><img class="mr-3 " width="60" src="images/no_images.png" alt="<?php echo $row["proje_baslik"]; ?>"><?php } ?></td>
											<td class=""><?php echo $row["proje_tarih"]; ?></td>
											<td class="sorting_1">
												<div class="d-flex">
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=files&id=<?php echo $row["proje_ust_id"]; ?>"><i class="fas fa-images" aria-hidden="true"></i></a>
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=edit&id=<?php echo $row["proje_ust_id"]; ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>
													<a class="contact-icon" href="<?php echo AREA; ?>?do=delete&id=<?php echo $row["proje_ust_id"]; ?>"><i class="fas fa-trash-alt"></i></a>
												</div>
											</td>
										</tr>
<?php
					...
					if(!empty($_POST['proje_baslik']) || !empty($_POST['proje_aciklama'])) {
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET proje_baslik = ?, proje_seo = ?, proje_aciklama = ?, proje_description = ?, proje_keywords = ?, proje_resim = ?, dil_id = ?, kategori_id = ?, proje_durum = ?, proje_ust_id = ?");
								$insert->execute(array($proje_baslik[$key], Seo_Link_Cevir($proje_baslik[$key]), $proje_aciklama[$key], $proje_description[$key], $proje_keywords[$key], $proje_resim, $key, $kategori_id, $proje_durum, $LastID));
													...
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																...
																<input type="text" name="proje_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Başlık" required="">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Ürün Fiyatı</label>
																<input type="text" name="proje_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Ürün Fiyatı" required="">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Kısa Açıklaması</label>
 																<textarea class="form-control" name="proje_keywords[<?php echo $LANGUAGE_CODE; ?>]"></textarea>
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
					...
					if(isset($submitControl)){
						if(!empty($_POST['proje_baslik']) || !empty($_POST['proje_aciklama'])) {
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									$key = str_replace("form_","",$k);
									$update = $db->prepare("UPDATE ".TABLE." SET  proje_baslik = ?, proje_seo = ?, proje_aciklama = ?, proje_description = ?, proje_keywords = ?, proje_resim = ?, proje_durum = ?, kategori_id = ? WHERE proje_ust_id = ? AND dil_id = ?");
									$update->execute(array($proje_baslik[$key], Seo_Link_Cevir($proje_baslik[$key]), $proje_aciklama[$key], $proje_description[$key], $proje_keywords[$key], $proje_resim, $proje_durum, $kategori_id, $id, $key));
								}
							}
							if ($update) {
...
?>
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														...
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Fiyat</label>
																<input type="text" name="proje_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Fiyat" required="" value="<?php echo GetTableValue("proje_description",TABLE,"where proje_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Kısa Açıklaması</label>
																<textarea class="form-control" name="proje_keywords[<?php echo $LANGUAGE_CODE; ?>]"><?php echo GetTableValue("proje_keywords",TABLE,"where proje_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?></textarea>
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
                        ...
                        <div class="card">
                            <div class="card-body p-3">
								...
								<div class="form-group">
									<label for="">Yayın Durumu</label>
									<select name="proje_durum" class="form-control">
										<option value="1" <?php if ($row_info["proje_durum"] == "1") { echo "selected";}?>>Aktif</option>
										<option value="2" <?php if ($row_info["proje_durum"] == "2") { echo "selected";}?>>Pasif</option>
									</select>
								</div>
                            </div>
                        </div>
...
<?php if($do == 'files'){ ?>
	...
	var myDropzone = new Dropzone("div#myDrop", {
	 ...
	 myDropzone.on("success", function(file, message){
		$("#msg").html(message);
		setTimeout(function(){
			window.location.href='projeler?<?php echo $_SERVER["REDIRECT_QUERY_STRING"]; ?>'},0
		);
	 });
