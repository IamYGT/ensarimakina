<?php require("../include/baglan.php");include("../include/fonksiyon.php");
		...
		define('TABLE',"arac_model");
		define('AREA',"modeller");
...
<?php
			if($do == 'marka') {
			$sayfa = (isset($q) ? $q : 1);
			$toplam_veri_sayisi = $db->query("SELECT COUNT(*) FROM ".TABLE." WHERE marka_id = {$marka_id} ")->fetchColumn();
		//	$limit = $ayarlar["strPageLimit"];
			$limit = 10;  
...
?>
					...
					<div style="margin-bottom: 15px;" class="col-xl-12">
						<a href="<?php echo AREA; ?>?do=add&marka&marka_id=<?php echo $marka_id ?>"><button style="float: right;"  type="button" class="btn btn-rounded btn-primary"><span class="btn-icon-left text-primary"><i class="fa fa-plus color-primary"></i></span>Yeni</button></a>
					</div>
					<div class="col-xl-12">
							...
							<div id="example5_wrapper" class="dataTables_wrapper no-footer">
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									<thead>
										...
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 45%">Marka</th>
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 45%">Model</th>
									...
									<tbody id="sortable" >
<?php
									$list = $db->query("SELECT * FROM ".TABLE." WHERE marka_id = {$marka_id} ORDER BY row ASC LIMIT $baslangic,$limit ");
										if ($list->rowCount()){
											foreach($list as $row){
											$marka_info = $db->query("SELECT * FROM arac_marka WHERE marka_id = {$marka_id}")->fetch(PDO::FETCH_ASSOC);
?>
										<tr id="item-<?php echo $row["model_id"]; ?>" role="row" class="even">
											...
											<td class="sorting_1">
												<div class="d-flex">
													<a class="contact-icon mr-3" href="yillar?do=model&model_id=<?php echo $row["model_id"]; ?>"><i class="fas fa-link" aria-hidden="true"></i></a>
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=edit&id=<?php echo $row["model_id"]; ?>&marka&marka_id=<?php echo $marka_id ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>
													<a class="contact-icon" href="<?php echo AREA; ?>?do=delete&id=<?php echo $row["model_id"]; ?>&marka&marka_id=<?php echo $marka_id ?>"><i class="fas fa-trash-alt"></i></a>
												</div>
											</td>
										</tr>
<?php
						...
						<?php if($toplam_veri_sayisi > $limit){ ?>
							<div class="dataTables_paginate paging_simple_numbers" id="example5_paginate">
								<?php
										...
										if($sayfa > 1){
											$onceki = $sayfa-1;
											echo '<a href="?do=marka&marka_id='.$marka_id.'&q='.$onceki.'" class="paginate_button previous disabled">Geri</a>';
										}
										if($sayfa==1){
											echo '<span><a class="paginate_button current" >1</a></span>';
										} else {
											echo '<span><a href="?do=marka&marka_id='.$marka_id.'&q=1" class="paginate_button previous">1</a></span>';
										}
										if($sayfa-$x > 2){
											echo '...';
											$i = $sayfa-$x;
										...
										for($i; $i<=$sayfa+$x; $i++) {
											if($i==$sayfa){
												...
												echo '<span><a href="?do=marka&marka_id='.$marka_id.'&q='.$i.'" class="paginate_button previous">'.$i.'</a></span>';
											}
											if($i==$sonSayfa) break;
										}
										if($sayfa < $sonSayfa){
											$sonraki = $sayfa+1;
											echo '<a href="?do=marka&marka_id='.$marka_id.'&q='.$sonraki.'" class="paginate_button next disabled">İleri</a>';
										}
								?>
							</div>
						<?php } ?>
...
<?php
				...
				if(isset($submitControl)){
					if(!empty($_POST['model'])) {
							$insert = $db->prepare("INSERT INTO ".TABLE." SET model = ?, marka_id = ?,  link = ?, gizli_ozellik = ?, model_durum = ?");
							$insert->execute(array($model, $marka_id, Seo_Link_Cevir($model), $gizli_ozellik, $model_durum));
							$last_id = $db->lastInsertId();
						if ($insert) {
							echo '<meta http-equiv="refresh" content="1;url='.AREA.'?do=edit&id='.$last_id.'&marka&marka_id='.$marka_id.'">';
								...
								<div class="row">
									<div class="col-lg-12 mb-2">
										<div class="form-group">
											...
											<input type="text" name="model" class="form-control" placeholder="Başlık" required="">
										</div>
									</div>
								</div>
                        ...
                        <div class="card">
                            <div class="card-body p-3">
								...
								<div class="form-group">
									...
									<select name="model_durum" class="form-control">
										<option value="1">Aktif</option>
...
<?php
				...
				$row_check = $db->prepare("SELECT * FROM ".TABLE." WHERE model_id = ?");
				$row_check->execute(array($id));
				if ($row_check->rowCount() > 0) {
					if(isset($submitControl)){
						if(!empty($_POST['model'])) {
							$update = $db->prepare("UPDATE ".TABLE." SET model = ?, marka_id = ?,  link = ?, gizli_ozellik = ?, model_durum = ?, model_id = ?");
							$update->execute(array($model, $marka_id, Seo_Link_Cevir($model), $gizli_ozellik, $model_durum, $id));
							if ($update) {
								echo '<meta http-equiv="refresh" content="1;url='.AREA.'?do=edit&id='.$id.'&marka&marka_id='.$marka_id.'">';
								$error = Bilgilendirme::Basarili("Başarılı şekilde Eklendi, görüntülemek üzere yönlendiriliyorsunuz..");
							}else {
								$error = Bilgilendirme::Hata("Bir Hata meydana geldi, daha sonra tekrar deneyiniz.");
							}
					...
					$row_info = $db->query("SELECT * FROM ".TABLE." WHERE model_id = {$id}")->fetch(PDO::FETCH_ASSOC);
?>
                            ...
                            <div class="card-body">
								<div class="row">
									<div class="col-lg-12 mb-2">
										<div class="form-group">
											<label class="text-label">Başlık</label>
											<input type="text" name="model" class="form-control" placeholder="Başlık" required="" value="<?php echo $row_info["model"]; ?>">
										</div>
									</div>
								</div>
								<div class="step-footer pull-right">
                        ...
                        <div class="card">
                            <div class="card-body p-3">
								...
								<div class="form-group">
									<label for="">Yayın Durumu</label>
									<select name="model_durum" class="form-control">
										<option value="1" <?php if ($row_info["model_durum"] == "1") { echo "selected";}?>>Aktif</option>
										<option value="2" <?php if ($row_info["model_durum"] == "2") { echo "selected";}?>>Pasif</option>
									</select>
								</div>
                            </div>
                        </div>
...
<?php
				...
				$row_check->execute(array($id));
				if ($row_check->rowCount() > 0) {
					$update = $db->exec("DELETE FROM ".TABLE." WHERE model_id = {$id};");
					if($update) {
						echo '<meta http-equiv="refresh" content="1;url='.AREA.'?do=marka&marka_id='.$marka_id.'">';
		...
		$( "#sortable" ).sortable({
			...
			stop: function (event, ui) {
				...
				$.ajax({
					...
					dataType: "json",
					data: data,
					url: "/include/ajax.php?do=carsmodel&action=row",
