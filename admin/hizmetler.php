							...
							<div id="example5_wrapper" class="dataTables_wrapper no-footer">
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									<thead>
										...
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Company: activate to sort column ascending" style="width: 215.6px;">Resim 370x493</th>
...
<?php
				...
				if(isset($submitControl)){
					if(!empty($_POST['haber_baslik']) || !empty($_POST['haber_aciklama'])) {
						...
						}$upload = new Upload($_FILES['haber_resim2']);
						if ($upload->uploaded) {
								$upload->file_auto_rename = true;
								$upload->process("../uploads/services");
							if ($upload->processed){
								$haber_resim2 = $upload->file_dst_name;
							} else {
								$error = Bilgilendirme::Hata("Hay Aksi! Bir hata meydana geldi.".$upload->error);
							}
						}
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET haber_baslik = ?, haber_seo = ?, haber_aciklama = ?, haber_description = ?, haber_kisaaciklama = ?,  haber_resim = ?,  haber_resim2 = ?, dil_id = ?, haber_durum = ?, haber_ust_id = ?");
								$insert->execute(array($haber_baslik[$key], Seo_Link_Cevir($haber_baslik[$key]), $haber_aciklama[$key], $haber_description[$key], $haber_kisaaciklama[$key], $haber_resim, $haber_resim2, $key, $haber_durum, $LastID));
						...
						if(!empty($_POST['haber_baslik']) || !empty($_POST['haber_aciklama'])) {
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									...
									$update = $db->prepare("UPDATE ".TABLE." SET  haber_baslik = ?, haber_seo = ?, haber_aciklama = ?, haber_description = ?, haber_kisaaciklama = ?,  haber_resim = ?,   haber_resim2 = ?, haber_durum = ? WHERE haber_ust_id = ? AND dil_id = ?");
									$update->execute(array($haber_baslik[$key], Seo_Link_Cevir($haber_baslik[$key]), $haber_aciklama[$key],  $haber_description[$key], $haber_kisaaciklama[$key],  $haber_resim,  $haber_resim2, $haber_durum, $id, $key));
								}
							}
							if ($update) {
